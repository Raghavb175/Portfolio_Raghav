...
const Navbar = () => {
    ...
    const scrollToSection = (ref) => {
        if (ref && ref.current) {
            window.scrollTo({
                top: ref.current.offsetTop - 100, // Adjusted offset for navbar height
                behavior: 'smooth'
            });
        }
    };
    ...
    return (
        <div className='flex w-full text-purple-600 justify-between pr-6'>
            <div className='logo ml-4 w-28 py-1 pl-px'><img src='./src/assets/Navimg/Logo.jpeg'></img></div>
            <ul className='flex gap-5 p-4 cursor-pointer left-[50%]'>
                <li onClick={() => scrollToSection(exampleRef)}>About</li>
                <li onClick={() => scrollToSection(projectsRef)}>Projects</li>
                <li onClick={() => scrollToSection(formRef)}>Contact</li>
                <li onClick={() => scrollToSection(carouselRef)}>Carousel</li>
            </ul>
            <ul className='flex gap-5 p-4 cursor-pointer'>
                <a href='https://github.com/Raghavb175' target='_blank' rel='noopener noreferrer'>
